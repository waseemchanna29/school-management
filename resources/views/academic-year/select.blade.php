...
<body>
    ...
    <div
        style="min-height:100vh; background:var(--light-bg);
        ...
        {{-- ── No Years Assigned Error (teachers only) ── --}}
        @if (!empty($noYearsError) && $noYearsError)
            <div style="width:100%; max-width:520px; text-align:center;">
                <div
                    style="background:var(--white); border-radius:var(--radius);
                box-shadow:var(--shadow-md); padding:3rem 2.5rem;">

                    <div
                        style="width:72px; height:72px; background:rgba(220,53,69,0.1);
                   ...
                   font-size:1.4rem; margin-bottom:0.6rem;">
                        No Academic Years Assigned
                    </h2>

                    <p
                        style="color:var(--text-muted); font-size:0.92rem;
                  line-height:1.6; margin-bottom:1.8rem;">
                        {{ $errorMessage ?? 'You have not been assigned any academic years yet.' }}
                        <br>
        ...
        <div style="text-align:center; margin-bottom:2.5rem; width:100%; max-width:780px;">
            <div
                style="width:64px; height:64px; background:var(--primary);
        ...
        {{-- Year Grid --}}
        <form action="{{ route('academic-year.select.post') }}" method="POST" style="width:100%; max-width:780px;">
            @csrf
            ...
            <div
                ...
                @foreach ($years as $year)
                    <div style="position:relative;">
                        <input type="radio" name="academic_year_id" id="year_{{ $year->id }}"
                            value="{{ $year->id }}" {{ $year->is_current ? 'checked' : '' }}
                        ...
                        <label for="year_{{ $year->id }}"
                            ...
                            id="lbl_{{ $year->id }}" onclick="selectYear({{ $year->id }})">
                            ...
                            <div
                                ...
                                <span class="badge {{ $year->status_badge_class }}" style="font-size:0.72rem;">
                                    {{ $year->status_label }}
                                </span>
                            </div>
                            ...
                            @if ($year->is_locked)
                                <div
                                    style="margin-top:6px; font-size:0.77rem;

...
function _smsEscHandler(e) { if (e.key === 'Escape') _smsHide(); }
...
function smsAlert(message, type = 'info', title = null) {
    const defaultTitles = { success:'Success', danger:'Error', warning:'Warning', info:'Notice' };
    ...
    _smsSetType(type);
    document.getElementById('sms-actions').innerHTML = `<button class="btn btn-primary" onclick="_smsHide()"><i class="fas fa-check"></i> OK</button>`;
    _smsShow();
}
function smsConfirm(message, onConfirm, title = 'Confirm Action', type = 'warning') {
    ...
    document.getElementById('sms-message').textContent = message;
    _smsSetType(type);
    const btnLabels = { danger:{confirm:'Yes, Delete',confirmClass:'btn-danger'}, warning:{confirm:'Yes, Continue',confirmClass:'btn-warning'}, info:{confirm:'Yes, Proceed',confirmClass:'btn-primary'}, success:{confirm:'Yes',confirmClass:'btn-success'} };
    ...
    document.getElementById('sms-confirm-btn').addEventListener('click', function() {
        _smsHide();
        if (typeof onConfirm === 'function') onConfirm(); else if (typeof onConfirm === 'string') eval(onConfirm);
    });
    _smsShow();
}
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('form[data-confirm]').forEach(form => {
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            smsConfirm(form.dataset.confirm || 'Are you sure?', () => form.submit(), form.dataset.title || 'Confirm Action', form.dataset.type || 'warning');
        });
    });
    @if(session('success')) smsAlert(@json(session('success')), 'success'); @endif
    @if(session('error'))   smsAlert(@json(session('error')),   'danger');  @endif
    @if(session('warning')) smsAlert(@json(session('warning')), 'warning'); @endif
    @if(session('info'))    smsAlert(@json(session('info')),    'info');    @endif
});
</script>
</body>
</html>